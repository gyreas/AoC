...
let get_subsets g = g.subsets

let overlap_subsets (ss1 : subset) (ss2 : subset) : int * int * int =
  let r, g, b = ss1 in
  let rr, gg, bb = ss2 in
  max r rr, max g gg, max b bb
;;

let minimum_colors (g : game) =
  let subsets = get_subsets g in
  let rec fold_colors_ acc lst =
    match lst with
    | [] -> acc
    | color :: rem ->
      let min = overlap_subsets color acc in
      fold_colors_ min rem
  in
  fold_colors_ (0, 0, 0) subsets
;;
